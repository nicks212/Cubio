...
/**
 * Natural, localized "a team member will follow up" handoff line for confirmed
 * human-escalation. Generated by Gemini so it VARIES and matches the CURRENT message's
 * language (never a single repeated hardcoded phrase, never history-derived). Falls back
 * to a deterministic, language-correct sentence only if generation fails.
 */
export async function generateEscalationHandoff(
  message: string,
  businessDescription: string | null | undefined,
  replyLanguage: 'ka' | 'en',
  usageContext?: Omit<AIUsageContext, 'feature' | 'model'>,
): Promise<string> {
  const phone = businessDescription
    ? /(?:\+?\d[\d\s\-()]{5,15}\d)/.exec(businessDescription)?.[0]?.trim() ?? null
    : null;
  const sys =
    `${LANGUAGE_RULE} ` +
    `The customer asked to speak with a human and you are confirming the handoff. ` +
    `Write ONE short, warm, natural sentence letting them know a team member will get back to them shortly. ` +
    `${phone ? `You may include this contact phone if it fits naturally: ${phone}. ` : ''}` +
    `Do not greet, do not apologize, add nothing else. Vary the wording naturally — never a canned line.`;
  try {
    ...
    if (text) {
      await persistAIUsage(
        usageContext ? { ...usageContext, feature: 'escalation_handoff', model: 'gemini-2.5-flash' } : null,
        result.response.usageMetadata,
      );
      return text;
    }
  } catch (err) {
    console.error('[ai/generate] escalation handoff generation failed:', err);
  }
  const phoneLine = phone ? `\n📞 ${phone}` : '';
  return replyLanguage === 'ka'
    ? `გთხოვთ ცოტა მოიცადოთ — ჩვენი გუნდი მალე დაგიკავშირდებათ 🙂${phoneLine}`
    : `One moment please — our team will get back to you shortly 🙂${phoneLine}`;
}
