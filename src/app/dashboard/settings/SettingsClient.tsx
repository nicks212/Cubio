...
function SuccessMsg({ state }: { state: { success?: boolean; error?: string } | null }) {
  if (!state) return null;
  if (state.error) return <p className="text-sm text-red-600">{state.error}</p>;
  if (state.success) return <p className="text-sm text-green-600 flex items-center gap-1.5"><CheckCircle className="w-4 h-4" />Saved successfully</p>;
  return null;
}
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        ...
        <form action={companyAction} className="space-y-4">
          ...
          <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-lg">
            ...
            />
            <div>
              <label htmlFor="ai_enabled" className="text-sm font-medium cursor-pointer">Enable AI Assistant</label>
              <p className="text-xs text-muted-foreground">AI will automatically respond to incoming messages</p>
            </div>
          </div>
          <div className="flex items-center justify-between">
            <SuccessMsg state={companyState} />
