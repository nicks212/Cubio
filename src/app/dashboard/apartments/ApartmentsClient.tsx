...
export default function ApartmentsClient({ apartments, projects, templates, companyId }: Props) {
  ...
  const openEdit = (a: ApartmentWithProject) => { setEditing(a); setModal('single'); };
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold text-foreground mb-2">Apartments</h1>
          <p className="text-muted-foreground">Manage your apartment inventory</p>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={() => { setEditing(null); setModal('single'); }} className="flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors font-medium text-sm">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-slate-200">
              <h2 className="text-xl font-semibold">{editing ? 'Edit Apartment' : 'Add Apartment'}</h2>
            ...
            <form action={editing ? updateAction : createAction} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-2">Apt Number *</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-2">Price/m² *</label>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={closeModal} className="flex-1 py-3 bg-slate-100 rounded-lg hover:bg-slate-200 transition-colors font-medium">Cancel</button>
                <button type="submit" disabled={editing ? updatePending : createPending} className="flex-1 py-3 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors font-medium disabled:opacity-50">
                  {(editing ? updatePending : createPending) ? 'Saving...' : 'Save'}
                </button>
              </div>
            </form>
          </div>
      ...
      {modal === 'bulk' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg">
            <div className="flex items-center justify-between p-6 border-b border-slate-200">
              <h2 className="text-xl font-semibold">Bulk Add Apartments</h2>
              <button onClick={closeModal} className="p-2 hover:bg-slate-100 rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <p className="text-sm text-muted-foreground bg-slate-50 rounded-lg px-4 py-3">
                Will create <strong>{(bulkForm.endFloor - bulkForm.startFloor + 1) * bulkForm.unitsPerFloor}</strong> apartments
              </p>
              <div className="flex gap-3">
                <button onClick={closeModal} className="flex-1 py-3 bg-slate-100 rounded-lg hover:bg-slate-200 font-medium">Cancel</button>
                <button onClick={handleBulkCreate} disabled={bulkLoading || !bulkForm.templateId || !bulkForm.projectId} className="flex-1 py-3 bg-primary text-white rounded-lg hover:bg-primary/90 font-medium disabled:opacity-50">
                  {bulkLoading ? 'Creating...' : 'Create Apartments'}
                </button>
              </div>
            </div>
          </div>
      ...
      {modal === 'template' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg">
            <div className="flex items-center justify-between p-6 border-b border-slate-200">
              <h2 className="text-xl font-semibold">Apartment Templates</h2>
              <button onClick={closeModal} className="p-2 hover:bg-slate-100 rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6">
              {/* Existing Templates */}
              {templates.length > 0 && (
                <div className="mb-6 space-y-2">
                  <h3 className="text-sm font-semibold text-muted-foreground mb-3">Existing Templates</h3>
                  {templates.map(t => (
                    <div key={t.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                      <div>
                        <p className="font-medium text-sm">{t.name}</p>
                        <p className="text-xs text-muted-foreground">{t.rooms_quantity}BR · {t.size_sq_m}m² · ${t.price_per_sq_m.toLocaleString()}/m²</p>
                      </div>
                      <button onClick={() => deleteTemplate(t.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg">
              ...
              <h3 className="text-sm font-semibold text-muted-foreground mb-3">Create New Template</h3>
              ...
              <form action={templateAction} className="space-y-3">
                <input name="name" required placeholder="e.g. 2BR Standard" className="w-full px-4 py-2.5 bg-[var(--input-background)] border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50" />
                <div className="grid grid-cols-3 gap-3">
                  <input name="size_sq_m" type="number" required placeholder="Size m²" className="w-full px-3 py-2.5 bg-[var(--input-background)] border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50" />
                  <input name="rooms_quantity" type="number" min="1" required placeholder="Rooms" className="w-full px-3 py-2.5 bg-[var(--input-background)] border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50" />
                  <input name="price_per_sq_m" type="number" required placeholder="Price/m²" className="w-full px-3 py-2.5 bg-[var(--input-background)] border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/50" />
                </div>
                <div className="flex gap-3">
                  <button type="button" onClick={closeModal} className="flex-1 py-3 bg-slate-100 rounded-lg hover:bg-slate-200 font-medium">Close</button>
                  <button type="submit" disabled={templatePending} className="flex-1 py-3 bg-primary text-white rounded-lg hover:bg-primary/90 font-medium disabled:opacity-50">
                    {templatePending ? 'Saving...' : 'Create Template'}
                  </button>
                </div>
              </form>
            </div>
